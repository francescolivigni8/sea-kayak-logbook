...
function createSparkline(values: number[], labels: string[], lineColor: string, fillColor: string): SparklineVisual {
    const width = 176;
    const height = 52;
...
function describeTemperature(value: number | null, min: number, max: number, coldLabel: string, warmLabel: string) {
    if (value === null) {
        return {
            percent: 0,
            minLabel: `${min}C`,
...
const monthlySeries = computed(() => props.monthlyDistance.map((row) => row.distanceKm));
const monthlyLabels = computed(() => props.monthlyDistance.map((row) => row.label));

const peakMonth = computed(() => {
    const fallback = props.monthlyDistance[0] ?? { key: 'none', label: 'No month', distanceKm: 0 };

    return props.monthlyDistance.reduce((best, row) => (
        row.distanceKm > best.distanceKm ? row : best
    ), fallback);
});

const averageSessionMinutes = computed(() => (
    props.headline.sessionCount ? (props.headline.durationHours * 60) / props.headline.sessionCount : 0
));
...
const durationBands = computed<BandVisual[]>(() => [
    ...
    },
    {
        ...
        percent: props.headline.sessionCount
            ? clamp((props.headline.trackSessions / props.headline.sessionCount) * 100, 0, 100)
            : 0,
...
const cards = computed<MetricCard[]>(() => [
    {
        ...
        detail: peakMonth.value.distanceKm > 0
    ...
    },
    {
        ...
        detail: props.context === 'public' ? 'Across public air logs' : 'Across logged air readings',
        style: 'linear-gradient(135deg, rgba(255,156,107,0.16), rgba(255,255,255,0.92))',
        type: 'temperature',
        thermometer: describeTemperature(props.seaState.temperatureAverages.air, -5, 25, 'Cool air', 'Warmer air'),
    },
    {
        ...
        detail: props.context === 'public' ? 'Across public sea logs' : 'Across logged sea readings',
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        >
            ...
            <div class="mt-4 flex items-end justify-between gap-3">
                ...
                <p
                    v-if="card.type === 'temperature' && card.thermometer?.hasValue"
                    class="rounded-full border border-[color:var(--journal-line)] bg-white/72 px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-[color:var(--journal-faint)]"
                >
            ...
            <div v-if="card.type === 'sparkline' && card.sparkline" class="mt-4">
                ...
                <div class="mt-2 flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.22em] text-[color:var(--journal-faint)]">
                    <span>{{ card.sparkline.startLabel }}</span>
                    <span>{{ card.sparkline.endLabel }}</span>
                </div>
            </div>

            <div v-else-if="card.type === 'bands' && card.bands" class="mt-4 grid gap-2.5">
                <div
                ...
                >
                    ...
                    <div class="h-2.5 overflow-hidden rounded-full bg-white/70">
                        <div
                            ...
                            :style="{ width: `${Math.max(band.percent, band.percent > 0 ? 10 : 0)}%`, background: band.fill }"
                        />
                    </div>
            ...
            <div v-else-if="card.type === 'temperature' && card.thermometer" class="mt-4 grid gap-2">
                <div class="relative h-3 overflow-hidden rounded-full bg-white/76 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]">
                    ...
                    <div
                        ...
                        :style="{ left: `calc(${card.thermometer.percent}% - 0.5rem)` }"
                    />
                </div>
